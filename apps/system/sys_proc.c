...
static int app_read(int off, char* dst);
...
int main() 
{
    ...
    while (1) 
    {
        ...
        case PROC_SETPRIO:
            ...
            */  
        {
            char pid = req->argv[0][0];
            char priority = req->argv[1][0];

printf("Setting %d to %d\n", pid , priority),
            grass->proc_set_priority((int)pid, (int)priority);
            grass->sys_send(sender, (void*)reply, sizeof(reply));
            break;
        }
        default:
            FATAL("sysproc: invalid request %d", req->type);
        

        case PROC_KILL:
        {
            char pid_char = req->argv[0][0];
            int pid = (int)pid_char; 

            // Set process priority here 
